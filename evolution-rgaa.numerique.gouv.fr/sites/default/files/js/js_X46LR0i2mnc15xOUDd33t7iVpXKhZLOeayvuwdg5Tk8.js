...
(function($) {	
  $(document).ready(function() {
  	// Expand/collapse on click on the whole header of accordion
    $('body').on('click', '.accordion-header', function (event) {
      event.preventDefault();
      var target = event.target;
      // only if it's not already clicking on the real button
      if ($(target).closest('.js-collapsible-cta').length === 0) {        
        $(this).find('.js-collapsible-cta').trigger('click');
      }      
    });
	});
  ...
  document.onreadystatechange = function () {
    if (document.readyState == "complete") {
       // Déplié automatique de l'accordion si hash renseigné avec le tid
      var hash = window.location.hash;
      if(hash.length > 0) {
        var theme_tid = hash.replace('#', '');
        $(".accordion-theme-tid-"	+ theme_tid).trigger('click');
      }
    }
  }
})(jQuery);;
